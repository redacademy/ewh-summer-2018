 ...
 */
  ...
  * @return bool
  */
 function ewh_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'ewh_categories' ) ) ) {
 		...
 		set_transient( 'ewh_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so ewh_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so ewh_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in ewh_categorized_blog.
  */
 function ewh_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'ewh_categories' );
 }
 add_action( 'edit_category', 'ewh_category_transient_flusher' );
 add_action( 'save_post',     'ewh_category_transient_flusher' );
  ...
  */
 function ewh_comment_list( $comment, $args, $depth ) {
  ...
  */
 function ewh_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
